
{
    ...
    function execute(input:string) {
        ...
        for ( const line of inputs ) {
            ...
            entries.push ( { signals: parts[0].split(" ").map( (signal) => signal.split("").sort() ), output: parts[1].split(" ").map( (signal) => signal.split("") ) } );
        }

        const digitRealSignals:Map<number, string> = new Map<number, string>();
        ...
        // sort() to make sure they are in alphabetical order for later
        digitRealSignals.set( 0, "abcefg" .split("").sort().join("") );
        digitRealSignals.set( 1, "cf"     .split("").sort().join("") );
        digitRealSignals.set( 2, "acdeg"  .split("").sort().join("") );
        digitRealSignals.set( 3, "acdfg"  .split("").sort().join("") );
        digitRealSignals.set( 4, "bcdf"   .split("").sort().join("") );
        digitRealSignals.set( 5, "abdfg"  .split("").sort().join("") );
        digitRealSignals.set( 6, "abdefg" .split("").sort().join("") );
        digitRealSignals.set( 7, "acf"    .split("").sort().join("") );
        digitRealSignals.set( 8, "abcdefg".split("").sort().join("") );
        digitRealSignals.set( 9, "abcdfg" .split("").sort().join("") );

        // return execute1(entries, digitRealSignals);
        return execute2(entries, digitRealSignals);
    }
    
    function execute1(entries:Array<Entry>, digitRealSignals:Map<number, string>):void {
        // part 1 is just seeing how many of 1, 4, 7 and 8 are in the output
        // so we just brute force that here

        let count = 0;
        for ( const testDigit of [1, 4, 7, 8] ) {
            const digitLength:number = digitRealSignals.get( testDigit )!.length;

            for ( const entry of entries ) {
                for ( const output of entry.output ) {
                    if ( output.length == digitLength ) {
                        count++;
                    }
                }
            }
        ...
        console.log("Result", count); // should be 476
    }
    
    function execute2(entries:Array<Entry>, digitRealSignals:Map<number, string>):void {
    
        // I don't really see an obvious, general-purpose algorithm here, so lets try to use some domain-knowledge and reasoning

        // for the example: acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf

        // for each of the 7 segments, we want to determine which of the letters maps to it
        // we start with the digit with the fewest segments (1 has 2 segments): that has to be "ab" in the example
        //    from the instructions, we know 1 should have C and F enabled. That means that C is either a or b, and F is either A or B (no way to know which just yet)
        // then we go to 7, which overlaps 1 with 2 segments (C and F) and adds 1 more (A). In the example, 7 is given as dab
        //    from the previous, we already know a and b map to C and F, so d is 100% sure to be A
        // sadly, there is no digit that adds just 1 to the 7... we only have 4, which adds B and D (but doesn't use A), or 3, which adds D and G
        //    let's try with 4 first. That has 4 segments, so has to be "eafb" in the example
        //    we again know a and b map to C and F, so e and f need to map to B and D
        //  now we can't just use 3, because that has 5 segments, and so do many other digits...
        //  all we can do is fall back to another thing we already know, which is 8
        //  however, that has EVERYTHING, and is basically useless... 

        //  A new approach is needed. Look for a digit that overlaps as much as possible with 4 or things we already know
        //  4 is BCDF and we know what A is. The digit that maps most to it is 9 (only has G on the bottom)
        //  as such, if we look at all the options with length of 6 (0, 6 and 9) and find the one that has 5 of the 6 signals overlapping with 4 + A, then we know what G is
        //      4 was eafb and A was d, so we're looking for something that mismatches "eafbd" by just one letter
        //      the three options are: cefabd, cdfgeb and cagedb
        //          if we put these things in alfabetical order, we get 4 = abdef. and the rest: abcdef, bcdefg and abcdeg
        // from this, we can see that abcdef is the one mismatching by only one letter (c) and so c maps to G (we now have a perfect mapping of both c to G and d to A)

        // Now, let's try to get another perfect mapping
        //  looking at digit 2, that has ACDEG, of which only E is something we haven't seen yet
        //      the other digits of length 5 (3 and 5) don't have E in them
        //  so, considering the three options of length 5 and looking for the one that has a signal in it that we haven't seen yet, will give us E
        //      options are cdfbe, fbcad and gcdfa. We have already seen eafbdc, so we're just missing the g! which corresponds to the last one, and g thus maps to E
        
        // so, we now know 2 is gcdfa. We also know 3 of the 5 signals for sure there: A, G and E. Only D and C are unknown
        // however, we already have boiled those to down to two options at the start: C is either a or b, and D is either e or f
        //  so, we can see for each which of the options is in the signals for 2!   
        //          2 is given as gcdfa. It has a but not b, so C = a. It has f but not e, so D = f.

        // we now have the following full mappings: A=d, C=a, D=f, E=g and G=c. This leaves us B and F to find.
        //  however, B was either e or f, and we know that D=f, so B=e
        //  by the powers of elimination, F then is the only leftover = b

        // decoding the 2 output numbers in the example (both are repeated):  cdfeb = GADBF = ABDFG = 5
        //      fcadb = DGCAF = ACDFG = 3
        // so the output is 5 3 5 3, which is indeed correct!

        // Now, all that's left is to implement this EXACT algorithm :) easy, right? 
        // though this seems a bit too complicated, no? 
        // is there really nothing more generalistic?
        ...
        // after some more thinking, I figured out we can get 100% certainty early on by powers of elimination

        // for 1 we start with ab
        // NEW INSIGHT: we can look at 6, 9 and 0 to figure out which of the two is C
        // 6, 9 and 0 are all 6 signals long, but only 6 has the C NOT set (they all have F set)
        //      so try both a and b for C and see which is only absent in 1 of the 3 6-signal options
        //  try a: 
        //       the 6 signal options are: cefabd, cdfgeb and cagedb => a is missing from just one, so C=a, F=b
        //  just to be sure, try b: 
        //       b is present in all three, so that's not it
        //      conclusion: C is a, F is b, for sure
        
        // then we also know what A is from 7 = d (the leftover)

        // for 4 we start with eafb
        // SAME insight: we can figure out which D is by looking at which is NOT in 0
            // D is the dash in the middle
            // 0, 6 and 9 all have 6 signals, but 0 is the only one where the D isn't set (B is set in all 3)
            // so if we take those three find out which of e or f isn't set in just 1, we fully know which is which already!

        // for 4, we again have two options for D: e and f
        //   try e : cefabd, cdfgeb and cagedb all have an e, so that's not it: this means D = f and B = e
        // no need to try f!

        // at this point, we have 5 of the 7 figured out, nice!
        // C=a, F=b, A=d, D=f, B=e

        // now we just need to get 1 more of either E or G (the other is the only remaining option)
        // 2, 3 and 5 all have G set, but only digit 2 also has E set!
        // so again take two options for E (the leftover ones): c and g
        //  try c: cdfbe, gcdfa, fbcad -> all three have c set, so E=g and G=c

        // BOOM, DONE!
        // let's implement that shall we:

        // 1. search 1 (digit of length 2)
        // 2. find 0, 6 and 9 (digits of length 6)
        // 3. for each signal in 1, count how many times it occurs in sixLongs : if count == 2, assign C and F
        // 4. search 7 (digit of length 2). Remove C and F, Assign A
        // 5. search 4 (digit of length 4). Remove C and F
        // 6. for each remaining signal in 4, count how many times it occurs in sixLongs: if count == 2, assign D and B
        // 7. find 2, 3 and 5
        // 8. start with all seen signals, remove the ones we have assigned.
        // 9. for each of the two remaining, count how many times it occurs in fiveLongs: if count == 2, assign E and G
        // 10. for each signal in output, substitute assigned values, compare those to the golden truth (reverse map to number)

        let sumResult = 0;

        // 1.
        for( const entry of entries ){
            // map of REAL SIGNAL -> input signal
            const assignedSignals:Map<string, string> = new Map<string,string>();

            const one:string[] = entry.signals.filter( (entry) => entry.length === 2 )[0]; // digit 1


            // 2.
            const sixLongs = entry.signals.filter( (entry) => entry.length === 6 ); // digits 0, 6 and 9

            // 3. 
            for ( const [idx, signal] of one.entries() ) {
                const signalPresent = sixLongs.filter( (digit) => digit.indexOf(signal) >= 0 );
                if ( signalPresent.length == 2 ) {
                    assignedSignals.set( "c", signal);
                    assignedSignals.set( "f", one[ (idx + 1) % one.length ]);

                    // console.log("Found C and F", assignedSignals);
                }
            }

            // 4.
            const seven:string[] = entry.signals.filter( (entry) => entry.length === 3 )[0]; // digit 7
            const sevenSignal = seven.filter( (signal) => [...assignedSignals.values()].indexOf(signal) < 0 )[0]; // just 1 leftover signal

            assignedSignals.set( "a", sevenSignal);
            // console.log("Found A", assignedSignals);

            // 5.
            const four:string[] = entry.signals.filter( (entry) => entry.length === 4 )[0]; // digit 4
            const fourSignals = four.filter( (signal) => [...assignedSignals.values()].indexOf(signal) < 0 ); // 2 leftover signals

            // 6.
            for ( const [idx, signal] of fourSignals.entries() ) {
                const signalPresent = sixLongs.filter( (digit) => digit.indexOf(signal) >= 0 );
                if ( signalPresent.length == 2 ) {
                    assignedSignals.set( "d", signal);
                    assignedSignals.set( "b", fourSignals[ (idx + 1) % fourSignals.length ]);

                    // console.log("Found D and B", idx, assignedSignals);
                }                
            }   

            // 7.
            const fiveLongs = entry.signals.filter( (entry) => entry.length === 5 ); // digits 2, 3 and 5

            // 8.
            let leftoverSignals = "abcdefg".split("").filter( (signal) => [...assignedSignals.values()].indexOf(signal) < 0 );

            // 9.
            for ( const [idx, signal] of leftoverSignals.entries() ) {
                const signalPresent = fiveLongs.filter( (digit) => digit.indexOf(signal) >= 0 );
                if ( signalPresent.length == 1 ) {
                    assignedSignals.set( "e", signal);
                    assignedSignals.set( "g", leftoverSignals[ (idx + 1) % leftoverSignals.length ]);

                    // console.log("Found E and G", idx, assignedSignals);
                }                
            }

            // 10.
            // assignedSignals now goes from REAL signals to input signals
            // for the output, we have to replace input signals with REAL signals, so need the inverted approach
            // same goes for the ground truth in digitRealSignals, where we want to go from the signals to the real numbers
            // FIXME: TODO: now that we know this, we can just build those maps up in the correct way from the start, so we don't have to do this
            //      but... this already took me way longer than I care to admit, so let's just leave it at this
            // https://stackoverflow.com/questions/56550463/invert-a-map-object
            const signalMap = new Map( Array.from(assignedSignals,  entry => [entry[1], entry[0]]) );
            const digitMap  = new Map( Array.from(digitRealSignals, entry => [entry[1], entry[0]]) );
            
            let outputNumbers= []
            for ( const output of entry.output ) {
                const outputMapped = output.map( (signal) => signalMap.get(signal) );

                outputNumbers.push( digitMap.get( outputMapped.sort().join("") ) );
            }

            sumResult += outputNumbers[0]! * 1000 + outputNumbers[1]! * 100 + outputNumbers[2]! * 10 + outputNumbers[3]!;
        }

        console.log( "Result", sumResult ); // should be 1011823
    }
